...
// representation of the alphabet - reverse of A is Z, reverse of B is Y, C is
// X, etc. This makes ciphering reciprocal - it is possible to use the exact
// same procedure for both encryption and decryption. It does not matter if you
// use the plain-letter/key-letter as a row or column (or vice versa) - you
// still get the same result. DIANA becomes failsafe when you are tired and
// under stress with the added bonus of being combat proven (US Special Forces
// during the Vietnam War and by US operatives during the Cold War). This made
// the DIANA cryptosystem an obvious choice for the Krypto431 system which this
...
func TrigraphByte(writeTo *byte, x *byte, y *byte) error {
	if writeTo == nil || x == nil || y == nil {
		return fmt.Errorf("trigraph: %w", ErrNilPointer)
	}
	if (*x < byte('A') || *x > byte('Z')) || (*y < byte('A') || *y > byte('Z')) {
		return fmt.Errorf("trigraph: %w", ErrOutOfRange)
	}
...
func AppendTrigraphByteByKey(writeTo *[]byte, character *byte, key *[]byte, keyIndex *int) error {
	switch {
	case writeTo == nil, character == nil, key == nil, keyIndex == nil:
		return ErrNilPointer
	}
	if *keyIndex < 0 {
		return ErrNegativeKeyIndex
	}
